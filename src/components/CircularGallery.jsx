...
async function loadFontFromStylesheet(url) {
  ...
  await Promise.allSettled(
    fontFaces.map(async face => { await face.load(); document.fonts.add(face); })
  );
  return family;
}
...
async function resolveFont(font, fontUrl) {
  ...
  try {
    ...
    if (document.fonts && document.fonts.load) {
      try { await document.fonts.load(resolved); } catch {}
    }
    return resolved;
  } catch (error) {
...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    const size = (this.viewport.height * (700 * this.scale)) / this.screen.height;
...
class App {
  ...
  addEventListeners() {
    ...
    window.addEventListener('mousewheel', this.boundOnWheel); window.addEventListener('wheel', this.boundOnWheel);
...
export default function CircularGallery({
  items, bend = 1, textColor = '#ffffff', borderRadius = 0.05,
  font = 'bold 30px Figtree', fontUrl,
